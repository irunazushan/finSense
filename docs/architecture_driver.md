...
### 3.6 Наблюдаемость
- Каждая транзакция имеет `correlation-id`. *(Correlation ID — это уникальный идентификатор, который позволяет отследить весь путь обработки одного запроса или события через систему)*
