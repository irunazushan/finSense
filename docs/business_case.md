...
- **Категоризация транзакций** часто выполняется с помощью упрощённых моделей, которые не учитывают контекст и могут ошибаться в нестандартных случаях.
...
1. **Быстрый ML-классификатор** на базе ONNX Runtime для массовой обработки транзакций с высокой уверенностью. Модель обучается и экспортируется с помощью Python training pipeline.
...
4. **Postgres с JSONB** для хранения структурированного ответа LLM в контуре рекомендаций.
5. **Раздельное хранение данных**: доменные данные и результаты обработки сохраняются в PostgreSQL, а AI-логи — в файловой системе для последующего анализа качества работы системы.
...
- **ML-модуль (ONNX Runtime + Python training pipeline):** модель классификации транзакций обучается в Python, экспортируется в ONNX и используется в `classifier-service` для быстрого синхронного инференса. Модуль возвращает категорию и числовую оценку уверенности.
- **Transaction Classifier Agent (LLM + RAG):** получает транзакции, в которых ML-модель не уверена (confidence < порога). Анализирует описание, историю пользователя и при сложных случаях обращается к базе похожих неоднозначных кейсов в `pgvector`, после чего возвращает категорию.
...
2. **Core service** читает топик, вызывает **ML-модуль**:
   ...
   - Иначе → транзакция отправляется в топик `llm-classifier-requests`.
3. **Transaction Classifier Agent** обрабатывает запрос:
   - Использует LLM с контекстом (история пользователя, merchant_name и т.д.).
   - Если случаям не хватает локального контекста, извлекает похожие сложные кейсы из `pgvector` и добавляет их в prompt.
...
Экспериментальные значения метрик будут рассчитаны на этапе оценки прототипа. В документации проекта зафиксированы целевые показатели и способы измерения, которые будут использованы в экспериментальной части.

В рамках прототипа confidence LLM используется как эвристическая оценка. 
| Метрика | Базовый ML/ONNX | Гибрид ML/ONNX + LLM | Способ измерения |
|--------|----------------------|----------------|-------------------|
| Точность категоризации | Только ML/ONNX | **>95%** (ML + LLM) | Сравнение с эталонной разметкой на тестовом наборе |
...
даже если нагрузка прототипа не требует полноценного брокера.*

### Почему ONNX Runtime и Python training pipeline для ML?
- Python training pipeline позволяет готовить синтетические и реалистичные датасеты, обучать модель, проводить оценку качества и экспортировать артефакты.
- ONNX Runtime позволяет использовать обученную модель в JVM-сервисе как быстрый и переносимый inference-компонент, сохраняя совместимость с микросервисной архитектурой на Spring.

### Почему Postgres с JSONB?
- Для структурированных данных (транзакции, пользователи) — реляционная модель.
- Для хранения структурированного ответа LLM в контуре рекомендаций — JSONB даёт гибкость без необходимости часто менять схему. Промпты, сырые ответы и технические метаданные при этом хранятся не в БД, а в файловых логах.

### Почему два агента?
- Разделение ответственности: Transaction Classifier Agent занимается классификацией и retrieval-обогащением сложных случаев, Financial Coach Agent — рекомендациями. Это упрощает разработку, тестирование и замену каждого агента независимо.
...
| Риск | Митигация |
|------|-----------|
| Низкое качество LLM-классификации (галлюцинации, неверные категории) | Использовать DeepSeek с ограничением допустимых категорий, добавить валидацию через confidence threshold, логировать все ответы для анализа |
