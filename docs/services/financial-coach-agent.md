...
- **Хранение рекомендаций** – сохранение структурированного ответа LLM в JSONB-поле таблицы `recommendations` для последующего отображения через Core API.
...
**4. Вызов LLM**
   Выполняется запрос к DeepSeek API, в ответ получается текст рекомендации и структурированные поля для `advice_data`.
...
**5. Обновление записи в БД**
   ...
   * при ошибке:
   ...
   В `advice_data` (JSONB) сохраняется только структурированный ответ LLM, предназначенный для последующей выдачи пользователю. Промпты, сырые ответы и технические метаданные в БД не сохраняются.
