...
* Идемпотентная обработка повторных сообщений в текущей реализации.
...
* Не обращается к PostgreSQL (в текущей реализации).
...
В текущей реализации используется простая стратегия:
