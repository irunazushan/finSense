...
### Бизнес-назначение
Transaction Classifier Agent обрабатывает транзакции, которые не удалось классифицировать с достаточной уверенностью с помощью быстрого ML/ONNX‑классификатора (Classifier Service). Агент использует большую языковую модель (LLM) для анализа контекста транзакции и истории пользователя, возвращает уточнённую категорию и уровень уверенности.
...
Сервис получает детали транзакции и историю пользователя через Kafka и результат классификации публикует так же в Kafka для дальнейшей обработки Core Service. В текущем прототипе агент не использует собственную БД и не читает контекст из PostgreSQL. Логи LLM хранятся исключительно в файлах.
...
- Не имеет собственного REST API, взаимодействует только через Kafka.
...
| **Service**              | `com.finsense.reasoning.service.ClassificationService`   | Координация процесса: обработка Kafka-запроса, вызов LLM, обработка ответа.      |
...
| `ClassificationService`            | Центральный сервис: использует контекст из Kafka-сообщения, вызывает LLM, обрабатывает результат. |
...
В дальнейшем агент может быть расширен отдельным хранилищем контекста или RAG-подходом для обогащения классификации. В текущем прототипе это не реализовано и не используется в рабочем потоке.
