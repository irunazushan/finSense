...
Сервис **читает** данные из общей БД (схема `core`), но **не изменяет** их напрямую. Он получает детали транзакции и историю пользователя, а результат классификации публикует в Kafka для дальнейшей обработки Core Service. Логи LLM хранятся исключительно в файлах, БД для этого не используется.
...
- Для каждого запроса обращается к БД, вызывает LLM, публикует ответ в `llm-classifier-responses`.
...
|-------------------------|--------------------------------|-----------------------------------------------------------------------------|
| Язык                    | Kotlin                         | Современный, лаконичный, корутины для асинхронности, единообразие с Core.   |
...
2. **Формирование промпта и вызов LLM**  
   `LLMService` строит промпт, используя:
    - Данные текущей транзакции (все поля)
    - Историю транзакций (массив до 20)
...
|---------------------------|--------------------------------------------------|-----------------------------------------|
| `llm-classifier-requests` | `{ trnsaction, history, historySize }`                      | Запрос на LLM‑классификацию             |

### Публикуемые события (Kafka)

| Топик                     | Тип сообщения                                    | Назначение                              |
|---------------------------|--------------------------------------------------|-----------------------------------------|
| `llm-classifier-responses`| `{ transactionId, category, confidence }`        | Результат классификации                  |
...
*Примечание:* все JPA-сущности маппятся на таблицы схемы `core`, изменения не производятся.
...
## 8. Обработка ошибок и ручная классификация

- При первом сбое или некорректном JSON выполняется повторный вызов LLM (до 3 попыток).
- Между попытками используется экспоненциальная задержка (backoff).
- Если после всех попыток ответ не получен или невалиден, в топик `llm-classifier-responses` отправляется сообщение с категорией `UNDEFINED` и `confidence = 0.0`. Core Service интерпретирует это как необходимость ручной классификации.
